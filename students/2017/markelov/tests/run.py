...
def runProg(bin, args, stdin = None):
    pipe = subprocess.Popen([bin] + args, stdin=subprocess.PIPE, stdout=subprocess.PIPE)
...
def runTest(mvm, root, test, doublerun, memcheck):
    if test in {'extern'}:
        return
    try:
        expectFile = None
        ...
        if doublerun:
            ...
            valgrind = 'valgrind'
            vagrind_args = ['--leak-check=full',  '--track-origins=yes', '--error-exitcode=1'] + [mvm, testFile]
            result = runProg(valgrind, vagrind_args)
...
def main(argv):
  ...
  for t in tests:
    ...
    if m is None:
      continue
    t = m.group(1)
    runTest(mvm, testdir, t, options.doublerun, options.memcheck)
