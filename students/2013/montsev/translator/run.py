...
def runTest(mvm, root, test, doublerun, generate):
    try:
        expectFile = None
        ...
        if doublerun:
            ...
            writeFile(expectFile, result)
            print 'File ' + expectFile + " has GENERATED"
            return
        else:
            expectFile = os.path.join(root, test+'.expect')
            result = runProg(mvm, testFile)
...
def main(argv):
  ...
  for t in tests:
    ...
    if m is None:
      continue
    t = m.group(1)
    runTest(mvm, testdir, t, options.doublerun, options.generate)
