...
class NewArrayInstanceNode : public AstNode {
  ...
  public:
    ...
    AstNode* dimAt(uint32_t position) const {
      return _dimensions[position];
    }

    virtual void visitChildren(AstVisitor* visitor) const {
      for (uint32_t i = 0; i < dimsCount(); i++) {
        dimAt(i)->visit(visitor);
      }
    }

    COMMON_NODE_FUNCTIONS(NewArrayInstanceNode);
};
...
class StoreByIndexNode : public AstNode {
  ...
  public:
    ...
    virtual void visitChildren(AstVisitor* visitor) const {
      indexNode()->visit(visitor);
      value()->visit(visitor);
    }

    COMMON_NODE_FUNCTIONS(StoreByIndexNode);
};
