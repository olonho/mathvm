...
void CodeVisitor::visitLoadNode(mathvm::LoadNode* node) {
  ...
  switch(info.type) {
    case VT_INT:
      ...
      cCode().addTyped(tranFuncs[curFuncId].getAddress(node->var()->name()));
      break;
    case VT_DOUBLE:
    ...
    case VT_STRING:
      ...
      break;
    default:
      transError("No code generated for load var " + node->var()->name() + );
  }
  cast(node);
}
  ...
  switch(node->op()) {
    case tASSIGN:
      if (type == VT_INT) {
        ...
        transError("No code generated for store var " + node->var()->name() + );
      }
      break;
    case tINCRSET:
      if (type == VT_INT) {
...
void CodeVisitor::visitForNode(mathvm::ForNode* node) {
  ...
  NodeInfo& rn = nodeInfo.getNodeInfo(op->right());
  ...
  if (opn.type == VT_INT) {
    ...
    op->right()->visit(this);
    cCode().addByte(BC_LOADIVAR);
    cCode().addByte(tranFuncs[curFuncId].getAddress(node->var()->name()));
    cCode().addBranch(BC_IFICMPG, lblEnd); 
  } else
  if (opn.type == VT_DOUBLE) {
    op->left()->visit(this);
    cCode().addByte(BC_STOREDVAR);
    cCode().addTyped(tranFuncs[curFuncId].getAddress(node->var()->name()));
    cCode().bind(lblLoopCheck);
...
void CodeVisitor::visitCallNode(mathvm::CallNode* node) {
  ...
  calledFunc.genCallingCode(callingFunc, this, cCode(), node);
  cast(node);
}
...
void CodeVisitor::visitFunctionNode(mathvm::FunctionNode* node) {
  ...
  code.addFunc(funcNodeToIndex[node], func);
  ...
  for (uint32_t i = 0; i < node->operands(); ++i) {
    ...
    switch (nop.type) {
      ...
      case VT_STRING:
        ...
        break;
      default:
        transError("Print: couldn't generate code");
        break;
    }
  }
  cast(node);
...
void  CodeVisitor::procBinNode(mathvm::TokenKind op, mathvm::VarType& resType) {
  using namespace mathvm;
