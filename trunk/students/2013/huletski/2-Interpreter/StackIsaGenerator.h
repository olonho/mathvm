...
public: // methods
  ...
  inline VarType toBroader(VarType top, VarType next) {
    ...
    long top_ind  = top == VT_DOUBLE ? 4 : top == VT_INT ? 2 : 1;
    long next_ind = next == VT_DOUBLE ? 4 : next == VT_INT ? 2 : 1;
    
    if (abs(top_ind - next_ind) == 3) { // D, S
  ...
  inline void pushInt(int64_t val) {
    switch (val) {
      case 1:addInsn(BC_ILOAD1); break;
      case -1: addInsn(BC_ILOADM1); break;
      case 0: addInsn(BC_ILOAD0); break;
      default:
        addInsn(BC_ILOAD);
        m_bc->addInt64(val);
        break;
...
private: // methods
  ...
  std::string indToStr(uint16_t ind) {
    switch (ind) {
      case 0: return "0";
      case 1: return "1";
      case 2: return "2";
  ...
  void genAccess(string const & type, VarInfo * v_i, uint16_t parent_func_scope) {
    if (v_i->scope_id != parent_func_scope) {
      ...
      if (v_i->local_ind < 4) {
        addInsn(type + t2s(v_i->type) + "VAR" + indToStr(v_i->local_ind));
      } else {
