...
variable_t introduce_var(AstNode* node, varmap_t& varmap, uint16_t context,
      std::string const& name, VarType type) {
...
variable_t introduce_var(AstNode* node, varmap_t& varmap, uint16_t context, AstVar* var) {
   return introduce_var(node, varmap, context, var->name(), var->type());
}
...
void pop_var(AstNode* node, varmap_t& varmap, variable_t const& v) {
   ...
   for(std::vector<variable_t>::iterator vit = mit->second.begin();
      ...
      if(vit->context_id == v.context_id) mit->second.erase(vit); return;
   }
   throw translator_exception::internal_error(node, "Can't find variable to pop");
}
...
public:
   void run(BytecodeImpl* code, Bytecode* ip, varmap_t& varmap, funmap_t& funmap,
      ...
      while(v.hasNext())
         locals.push_back(introduce_var(node, _varmap, _context, v.next()));
   ...
   virtual void visitForNode(ForNode* node) {
      ...
      variable_t lo_var = introduce_var(node, _varmap, _context, "%lo", VT_INT);
      variable_t hi_var = introduce_var(node, _varmap, _context, "%hi", VT_INT);
   ...
   virtual void visitFunctionNode(FunctionNode* node) {
      ...
      variable_t context = introduce_var(node, _varmap, _context, "@context", VT_INT);
      ...
      decrement_context(f.ip, context.id);
   }
