...
AstNode *Parser::parseIndex() {
    uint32_t tokenIndex = _currentTokenIndex;
    assert(currentToken() == tIDENT);
    assert(lookaheadToken(1) == tLBRACKET);
    AstVar* arrayref = _currentScope->lookupVariable(currentTokenValue());
    consumeToken();

    vector<AstNode*> idxs;
    while (currentToken() == tLBRACKET) {
...
StoreByIndexNode* Parser::parseAssignmentByIndex(AstNode* lhs) {
    ...
    StoreByIndexNode* result = new StoreByIndexNode(_currentTokenIndex,
                                                    lhs,
                                                    parseExpression(),
                                                    op);
...
void Parser::parseArrayref(VarType& type) {
  uint32_t dims = 0;
  while (currentToken() == tLBRACKET) {
    ensureToken(tLBRACKET);
    ensureToken(tRBRACKET);
...
AstNode* Parser::parseNewExpr() {
    uint32_t tokenIndex = _currentTokenIndex;
    assert(currentToken() == tIDENT && currentTokenValue() == "new");
    consumeToken();

    VarType valueType = toPrimitiveType(currentTokenValue());
    if (currentToken() != tIDENT && (valueType == VT_INVALID)) {
    ...
    ensureToken(tSEMICOLON);
    return new NewArrayInstanceNode(tokenIndex, dims, valueType);
}
...
AstNode* Parser::parseUnary() {
    if (isUnaryOp(currentToken())) {
        ...
        return expr;
    } else if (currentToken() == tIDENT && !isKeyword(currentTokenValue()) && lookaheadToken(1) == tLBRACKET) {
        AstNode* expr = parseIndex();
        return expr;
    } else if (currentToken() == tIDENT && currentTokenValue() == "new") {
        return parseNewExpr();
